...
class TestRaffle(unittest.TestCase):
    ...
    def test_option_1(self):
        # Start a new draw
        self.raffle.start_new_draw()
        self.assertEqual(self.raffle.pot, 100)
        self.assertEqual(self.raffle.tickets, {})
        self.assertEqual(self.raffle.draw_status, "Draw has not started")

    def test_option_2(self):
        # Buy tickets
        self.raffle.buy_tickets('Alice', 3)
        self.assertEqual(self.raffle.pot, 115)
        self.assertEqual(len(self.raffle.tickets['Alice']), 3)
        
        # Check max limit
        self.raffle.buy_tickets('Alice', 3)
        self.assertEqual(len(self.raffle.tickets['Alice']), 5)

    def test_option_3(self):
        # Simulating a raffle run
        self.raffle.buy_tickets('Bob', 5)
        self.raffle.run_raffle()
        self.assertEqual(self.raffle.draw_status, "Draw has ended")
        
        # As we can't predict the winning ticket, we'll just verify the pot value
        self.assertGreaterEqual(self.raffle.pot, 100)
